"""All FastMCP tool registrations for the Word Document MCP server.

Extracted from main.py so the entry point stays a lean coordinator.
Each `@mcp.tool` decorator binds an MCP-visible tool to a thin wrapper
that delegates to the relevant `tools/<module>.py`.
"""
...
def register_all_tools(mcp):
    """Register every @mcp.tool with the given FastMCP instance."""
    ...
    def list_tracked_changes(filename: str):
        """[DEPRECATED for production: prefer word_live_list_revisions which reads Word's native revision list.]
    ...
    def accept_tracked_changes(filename: str, author: str = None, change_ids: list[int] = None):
        """[DEPRECATED for production: prefer word_live_accept_revisions which uses Word's native revision engine.]
    ...
    def reject_tracked_changes(filename: str, author: str = None, change_ids: list[int] = None):
        """[DEPRECATED for production: prefer word_live_reject_revisions which uses Word's native revision engine.]
    ...
    @mcp.tool(annotations=ToolAnnotations(title="Audit Document Against Spec", readOnlyHint=True))
    async def word_audit_against_spec(
        filename: str,
        spec: list = None,
        spec_path: str = None,
    ):
        """Audit a .docx against a structured formatting spec — the verification
        layer that decouples "AI says done" from "actually done".

        Each spec item is ``{"id": "<dotted.path>", "expected": <value>}``. The
        engine reads the document, runs the matching checker for each id, and
        returns ``{pass_count, fail_count, items: [...]}`` where every failing
        item carries ``expected`` / ``actual`` / ``fix_hint``.

        Usage pattern:
            1. After making edits, call ``word_live_save`` so on-disk state matches
               what you intend to audit.
            2. Call this tool with either an inline ``spec`` list or a ``spec_path``
               pointing at a .json file.
            3. If ``fail_count > 0``, fix the listed items and re-audit.
            4. Surface the final audit JSON to the user before claiming completion.

        Supported id namespaces (extend in tools/audit_tools.py::CHECKERS):
            page.*       — margins, gutter, header/footer distance, odd/even pages
            body.*       — font, size, line spacing on Normal-style paragraphs
            heading1.*   — font, size, alignment on Heading 1 style
            heading2.*   — same on Heading 2
            headings.*   — cross-cutting checks (level skipping)
            patterns.*   — whole-document scans (blank runs, forbidden fonts,
                           red text, half-width punctuation in CJK, keyword sep)
            references.* — numbering format

        Args:
            filename: Path to the .docx (must be saved to disk).
            spec: Inline list of {id, expected} dicts. Mutually exclusive with spec_path.
            spec_path: Path to a JSON spec file. Mutually exclusive with spec.
        """
        return await audit_tools.word_audit_against_spec(filename, spec=spec, spec_path=spec_path)
